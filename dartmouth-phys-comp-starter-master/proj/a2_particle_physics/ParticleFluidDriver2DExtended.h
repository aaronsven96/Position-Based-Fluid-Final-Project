...
//#####################################################################
#ifndef __ParticleFluidDriver2DExtended_h__
#define __ParticleFluidDriver2DExtended_h__
#include <random>
#include "Common.h"
#include "Driver.h"
#include "Particles.h"
...
public:
	...
	virtual Array<VectorD> getParticleArray(double rad){
        ...
        for(int i=0;i<n;i++){
            ...
            pos[0]=center[0]+r*cos(theta);
            pos[1]=center[1]+r*sin(theta);
...
template<int d> class ParticleFluidDriver2DExtended : public Driver, public OpenGLViewer
{
...
std::cout << "added particles" << std::endl;
		//bowl = new Bowl<d>(VectorD::Unit(1) * 10, 16);
		sphereb = new Sphereb<d>(VectorD::Unit(1) * 10, 16);
		sphere = new Sphere<d>(VectorD::Unit(1), 2);
		sphere->center[0] += 1000;


	std::cout << "created empty sphereb" << std::endl;
		Array<VectorD> boundpos = sphereb->getParticleArray(0.008);
		for(int j=0;j<boundpos.size();j++){
			Add_Particle_Boundary(boundpos[j]);
			//fluid.is_boundary.push_back()
			//std::cout << "x:"<<boundpos[j][0]<<"y:"<<boundpos[j][1] << std::endl;
		}
	...
	Define_Function_Object(ParticleFluidDriver2DExtended, Keyboard_Event_S);
	Define_Function_Object(ParticleFluidDriver2DExtended, Keyboard_Event_A);
	Define_Function_Object(ParticleFluidDriver2DExtended, Keyboard_Event_W);
	Define_Function_Object(ParticleFluidDriver2DExtended, Keyboard_Event_D);
	Define_Function_Object(ParticleFluidDriver2DExtended, Keyboard_Event_Q);
	Define_Function_Object(ParticleFluidDriver2DExtended, Keyboard_Event_E);
...
protected:
	void Add_Particle(VectorD pos, real m = 1.)
	{
		...
		fluid.is_boundary.push_back(false);
		fluid.particles.R(i) = .1;
		fluid.particles.M(i) = m;
		fluid.particles.D(i) = 1.;
	}
	void Add_Particle_Boundary(VectorD pos, real m = 1.)
	{
		int i = fluid.particles.Add_Element();	////return the last element's index
		fluid.particles.X(i) = pos;
		fluid.particles.V(i) = VectorD::Zero();
		fluid.is_boundary.push_back(true);
		fluid.particles.R(i) = .1;
		fluid.particles.M(i) = m;
		fluid.particles.D(i) = 1.;
	}
